...
func (h *Handler) GetArticles(ctx *gin.Context) {
	...
	offset := (page - 1) * limit
	articles, err := h.services.ArticleService.GetFeaturedArticles(ctx.Request.Context(), limit, offset)
	if err != nil {
...
// GetArticlesAdmin godoc
// @Summary Get all articles for admin
// @Description Get all articles for admin panel (including non-featured)
// @Tags articles
// @Accept json
// @Produce json
// @Security BearerAuth
...
// @Failure 400 {object} map[string]string
// @Failure 500 {object} map[string]string
// @Router /articles/admin [get]
func (h *Handler) GetArticlesAdmin(ctx *gin.Context) {
	pageVal, err := strconv.ParseInt(ctx.DefaultQuery("page", "1"), 10, 32)
	if err != nil {
		h.logger.Error("failed to parse page parameter", zap.String("page", ctx.Query("page")), zap.Error(err))
		ctx.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "Invalid page parameter"})
	...
	if err != nil {
		h.logger.Error("failed to get articles for admin", zap.Int32("limit", limit), zap.Int32("offset", offset), zap.Error(err))
		h.handleError(ctx, err)
		return
	}
	ctx.JSON(http.StatusOK, dto.MapToArticleSummaryDTOs(articles))
